...
def copy_state(toState, fromState):    
    if fromState != False:
        for (name,val) in vars(fromState).items():
            if name != '__name__':     
				
				exec('keyList=fromState.'+name)
				if len(keyList) > 0:
					exec('toState' + '.' + name + '={}')
				for keys in keyList:
					#toState.name[key] = fromState.name[key]
					exec('toState.' + name + '[' + keys + ']=fromState.' + name + '[' + keys + ']')
					#exec('toState' + '.' + name + '=' + 'fromState' + '.' + name)
    else:
		return False
